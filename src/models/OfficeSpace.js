import { Schema } from 'mongoose';
import { mongoSchemaOptions } from '../utilities/constants.js';


const OfficeSpaceSchema = new Schema({
        name: { type: String, required: true },
        description: String,
        slug: String,
        ratings: String,
        spaceTag: String,
        space_contact_details : {
            name: String,
            email: String,
            phone: String,
            show_on_website: Boolean,
            },
        builder: {
            type: Schema.Types.ObjectId,
            ref: 'Builder'
        },
        building: {
            type: Schema.Types.ObjectId,
            ref: 'SubBuilder'
        },
        likes: [{
            type: Schema.Types.ObjectId,
            ref: 'User'
        }],
        contact_details: [{
            designation: String,
            name: String,
            phone_number: String
        }],
        hours_of_operation: {
            is_sunday_open: { type: Boolean, default: false },
            is_open_24: { type: Boolean, default: false }
        },
        location: {
            name: String,
            floor: String,
            address: String,
            country: {
                type: Schema.Types.ObjectId,
                ref: 'Country'
            },
            state: {
                type: Schema.Types.ObjectId,
                ref: 'State'
            },
            city: {
                type: Schema.Types.ObjectId,
                ref: 'City'
            },
            micro_location: [{
                type: Schema.Types.ObjectId,
                ref: 'MicroLocation'
            }],
            property_id: String,
            state: String,
            country: String,
            postal_code: String,
            landmark: String,
            landmark_distance: String,
            metro_stop_landmark: String,
            ferry_stop_landmark: String,
            ferry_stop_distance: String,
            bus_stop_landmark: String,
            bus_stop_distance: String,
            taxi_stand_landmark: String,
            taxi_stand_distance: String,
            tram_landmark: String,
            tram_distance: String,
            hospital_landmark: String,
            hospital_distance: String,
            school_landmark: String,
            school_distance: String,
            restro_landmark: String,
            restro_distance: String,
            latitude: Number,
            longitude: Number,
            metro_detail: {
                name: String,
                is_near_metro: {
                    type: Boolean,
                    default: false
                },
                distance: Number
            },
            micro_location_string: {
                type: String
            },
            is_near_metro: {
                type: Boolean,
                default: false
            },
            is_ferry_stop: {
                type: Boolean,
                default: false
            },
            is_bus_stop: {
                type: Boolean,
                default: false
            },
            is_taxi_stand: {
                type: Boolean,
                default: false
            },
            is_tram: {
                type: Boolean,
                default: false
            },
            is_hospital: {
                type: Boolean,
                default: false
            },
            is_school: {
                type: Boolean,
                default: false
            },
            is_restro: {
                type: Boolean,
                default: false
            },
        },
        amenties: [{
            type: Schema.Types.ObjectId,
            ref: 'Amenty'
        }],
        other_detail: {
            building_name: String,
            how_to_reach: String,
            area_for_lease_in_sq_ft: Number,
            rent_in_sq_ft: Number,
            monthly_rent: Number,
            floor: {
                type: String
            },
            security_deposit: {
                type: String
            },
            monthly_maintenance: {
                type: String
            },
            monthly_maintenance_amount: {
                type: String
            },
            office_type: {
                type: String,
                enum: ['semi-furnished', 'fully-furnished', 'raw', 'build-to-suite'],
                default: 'fully-furnished'
            },
            facilities: [{
                name: String,
                value: String
            }]
        },
        social_media: {
            twitter: {
                type: String
            },
            facebook: {
                type: String
            },
            instagram: {
                type: String
            }
        },
        seo: {
            title: { type: String },
            description: { type: String },
            robots: String,
            keywords: String,
            url: String,
            script: String,
            status: {
                type: Boolean,
                default: true
            },
            twitter: {
                title: String,
                description: String,
                image: {
                    type: Schema.Types.ObjectId,
                    ref: 'Image'
                }
            },
            open_graph: {
                title: String,
                description: String,
                image: {
                    type: Schema.Types.ObjectId,
                    ref: 'Image'
                }
            }
        },
        geometry: {
            type: { type: String, default: 'Point' },
            coordinates: { type: [Number] }
        },
        images: [{
            image: {
                type: Schema.Types.ObjectId,
                ref: 'Image'
            },
            order: Number
        }],
        is_active: {
            type: Boolean,
            default: true
        },
        status: {
            type: String,
            enum: ['pending', 'approve', 'reject', 'inprogress'],
            default: 'pending'
        },
        assignedUsers: [{ type: Schema.Types.ObjectId, ref: 'User' }],
         planStatus: { type: String },
        is_popular: {
            value: {
                type: Boolean,
                default: false
            },
            order: {
                type: Number
            }
        },
        priority_loc: [
            {
              is_active: {
                type: Boolean,
                default: false,
              },
              order: {
                type: Number,
                default: 1000,
              },
              microlocationId: {
                type: Schema.Types.ObjectId,
                ref: "MicroLocation",
              },
            },
          ],
        priority: {
            overall: {
                is_active: {
                    type: Boolean,
                    default: false
                },
                order: {
                    type: Number,
                    default: 1000
                }
            },
            location: {
                city: {
                    type: Schema.Types.ObjectId,
                    ref: 'City'
                },
                is_active: {
                    type: Boolean,
                    default: false
                },
                order: {
                    type: Number,
                    default: 1000
                }
            },
            micro_location: {
                name: String,
                city: {
                    type: Schema.Types.ObjectId,
                    ref: 'City'
                },
                is_active: {
                    type: Boolean,
                    default: false
                },
                order: {
                    type: Number,
                    default: 1000
                }
            }
        },
        user: {
            type: Schema.Types.ObjectId,
            ref: 'User',
            default: '5f2ce468ecdb5a5d67f0c621'
        },
        createdAt: { type: Date, default: Date.now },
        expireAt: { type: Date, default: Date.now },
        productId: { type: String, default: '' },
        added_by_user: {
            type: String,
            enum: ['seller', 'admin'],
            default: 'admin'
        },
        space_type_key: { type: String }
    },
    mongoSchemaOptions
)

OfficeSpaceSchema.virtual('space_type').get(function() {
    return "office-space/rent";
});
OfficeSpaceSchema.index({ geometry: "2dsphere" });
OfficeSpaceSchema.index({ "name": 1, "location.address": 1, "location.property_id": 1 }, { unique: true })


export default OfficeSpaceSchema;